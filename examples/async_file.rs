...
impl Future for AsyncWriteFuture {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match guard.write_all(lined_entry.as_bytes()) {
            Ok(_) => println!("wiritten for: {:?}", self.entry),
            Err(error) => {
                println!("{}", error)
            }
        };
...
// This function is used to write a log entry to the file asynchronously
...
async fn main() {
    ...
    for name in names {
        ...
        let handle2 = write_log(file_handle2, name.to_string());
        handles.push(handle);
        handles.push(handle2);
    }
    let _ = join_all(handles).await;

    // log content is not ordered, in async thread mode, the order is not guaranteed.
}
